...
<template>
  <div v-if="show">
    ...
    >
      ...
      <div v-if="dancers.length" class="text-[13px] font-semibold text-black/45 mt-0.5">{{ dancers.join(' & ') }}</div>
    </div>
    ...
    >
      ...
      <div class="text-xs text-gold-400/50 mt-0.5">
        {{ targetDayLabel }}<span v-if="time"> · {{ time }}</span><span v-if="subtitle && !isWatchedAwards"> · {{ subtitle }}</span><span v-if="isWatchedAwards && dancers.length"> · {{ dancers.join(' & ') }}</span>
      </div>
    </div>
    ...
    >
      ...
      <div class="text-xs text-gold-400/50 mt-0.5">
        <template v-if="isWatchedAwards">
          <span v-if="dancers.length">{{ dancers.join(' & ') }}<span v-if="time"> · {{ time }}</span></span>
          <span v-else>{{ time }}</span><span v-if="timeDiff"> · {{ timeDiff }}</span>
        </template>
        <template v-else>
