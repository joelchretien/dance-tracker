...
<template>
  <div v-if="show">
    ...
    >
      ...
      <div class="text-[13px] font-semibold text-black/45 mt-0.5">Your dancers are in this block</div>
    </div>
    ...
    >
      ...
      <div class="text-xs text-gold-400/50 mt-0.5">
        <template v-if="isWatchedAwards">
          Your dancers are in this block<span v-if="time"> · {{ time }}</span><span v-if="timeDiff"> · {{ timeDiff }}</span>
        </template>
        <template v-else>
