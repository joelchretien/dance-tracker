    ...
    <template v-else v-for="(li, i) in listItems" :key="`${li.kind}-${li.globalIndex ?? i}`">
      ...
      <template v-else-if="li.kind === 'entry'">
        <DanceEntry
          ...
          :same-studio="!!((schedule.flatEntries[li.globalIndex!].entry as DanceEntryType).studio && watchStore.watchedStudios.has((schedule.flatEntries[li.globalIndex!].entry as DanceEntryType).studio!))"
